...
const USER_ID = 4; // Static User ID used in your working version
...
const getCollectedIds = async () => {
    const data = await fetchData(`frankendael_users_plants?filter[frankendael_users_id]=${USER_ID}&fields=frankendael_plants_id`);
...
const getCollectedPlants = async () => {
    const data = await fetchData(`frankendael_users_plants?filter[frankendael_users_id][_eq]=${USER_ID}&fields=*,frankendael_plants_id.*.*`);
...
app.get('/', async (request, response) => {
    try {
        ...
        const userProfile = await fetchData(`frankendael_users/${USER_ID}`);
...
app.get('/veldverkenner/:zone_slug', async (request, response) => {
    try {
        ...
        const plantsInZone = plantIds.length ? await fetchData(`frankendael_plants?filter[id][_in]=${plantIds.join(',')}&fields=*.*`) : [];
        ...
        const normalizedPlants = plantsInZone.map(plant => {
            ...
            return { 
                ...
                // This is the specific logic from your working snippet:
...
app.get('/veldverkenner/:zone_slug/:item_slug', async (request, response) => {
    try {
        ...
        response.render('opdracht.liquid', { quest: plant, plant, zone: zoneData[0], zone_slug: request.params.zone_slug, state: request.query.step || 'intro', user_id: USER_ID, zone_type: zoneData[0].type, current_path: request.path });
    } catch (error) { response.status(500).send("Quest error"); }
});

app.get('/collectie', async (request, response) => {
    const [collected, allZones] = await Promise.all([getCollectedPlants(), fetchData('frankendael_zones')]);
...
app.get('/collectie/not_in_bloom', async (request, response) => {
    const collected = await getCollectedPlants();
...
app.get('/account', async (request, response) => {
    try {
        const [userData, collectedPlants] = await Promise.all([
            fetchData(`frankendael_users/${USER_ID}`),
            getCollectedPlants(USER_ID)
        ]);

        const totalCollected = collectedPlants.length;

        response.render('account.liquid', { 
            user: userData, 
            total_plants: totalCollected,
            current_path: request.path 
        });
    } catch (error) {
...
app.post('/veldverkenner/:zone_slug/:item_slug', async (request, response) => {
    ...
    try {
        await fetch(`${API_BASE}/frankendael_users_plants`, {
            ...
            body: JSON.stringify({ frankendael_users_id: USER_ID, frankendael_plants_id: parseInt(plant_id, 10) })
        });
